        ...
        try {
            ...
            while (buffer_pos < bytes_transferred) {
                ...
                if (self->_isParsed == false) {
                    ...
                    // 计算本次可以读取的头部字节数
                    size_t head_to_copy = std::min(head_remaining, bytes_transferred - buffer_pos);
                ...
                if (self->_isParsed) {
                    // 计算还需要读取多少字节的消息体
                    size_t body_remaining = self->_msg_body->totalLen - self->_msg_body->currentLen;
                    // 计算本次可以读取的消息体字节数
                    size_t body_to_copy = std::min(body_remaining, bytes_transferred - buffer_pos);
                    ...
                    std::cout << "receive msg body length:" << self->_msg_body->totalLen << std::endl;
                    std::cout << "receive msg body:" << std::string(self->_msg_body->data, self->_msg_body->totalLen) << std::endl;
