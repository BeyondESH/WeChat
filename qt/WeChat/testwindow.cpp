...
std::vector<QString> testWindow::generateMsg()
{
    std::vector<QString>  strs ={"你好",
                                 "This is the way",
                                 "I love youmessageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离messageWIdget作为item添加到msgLIst里后textedit的高度还是存在问题，我发现messagewidget整体宽度变大但是textedit的高度并没有变化，我希望的是textedit的高度在变大时，其上部分在相对于headlb的位置不变，其下部分拉长，并可以超出headlb的长度距离",
